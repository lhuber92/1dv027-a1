...
export default function Layout({ staticProps, error, isLoggedIn, isHome, children, userdata = false }) {
  ...
  const logIn = async () => {
    ...
    const authURL = `https://gitlab.lnu.se/oauth/authorize?client_id=${staticProps.appId}&redirect_uri=${staticProps.redirectUrl}&response_type=code&state=${state}&scope=read_user`
    router.push(authURL)
  }
